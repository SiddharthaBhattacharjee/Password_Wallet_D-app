...
function App(){
  const [Data, setData] = useState([]);
    ...
    const updateHandler = async (key_, siteName, username, password) =>{
      console.log('updating key : ',key_);
      ...
      try{
        ...
        if(ethereum){
          ...
          const PWContract = new ethers.Contract(PWContractAddress, pwAbi.abi, signer);//this step is to connect to the smart contract
          //calling the smart contract
          let id_ = key_.toString();
          PWContract.updateData(key_,data.sitename, data.username, data.password, data.isDeleted)
          .then(response => {
            getAllData();
          })
          .catch(err => {
